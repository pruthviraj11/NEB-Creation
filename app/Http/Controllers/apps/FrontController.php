...
//   }
  ...
  public function photos($slug = null)
  {
    ...
    if ($slug) {
      ...
      if ($category) {
        // Get photos filtered by the category id and active status
        $photos = Photography::where('category_id', $category->id)
          ->where('status', '1')
          ->orderBy('id', 'desc')
          ->get();
      } else {
        // If category slug not found, return empty collection or handle as needed
        $photos = collect([]);
      }
    } else {
      // If no slug provided, fetch all active photos
      $photos = Photography::where('status', '1')->orderBy('id', 'desc')->get();
    }
